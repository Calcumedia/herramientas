...
const [html,app,v45,v46css,v46,health,danger,dangerMap,previfoc,previfocSource,airQuality,perimeters,roads,situation,infoca,infocaSource,bombers,bombersSource,infoar,infoarSource,galiciaSource,asturiasSource,murciaSource,weather,pkg,lock,manifest,readme,deployment,vercelConfig] = await Promise.all([
  ...
  readFile(join(root,'sources/asturias-source.js'),'utf8'),
  readFile(join(root,'sources/murcia-source.js'),'utf8'),
...
assert.match(health,/integratedRegionalTerritories:8/);
assert.match(health,/infocaEndpoint:true/);
assert.match(health,/infocaOfficialSource:true/);
assert.match(health,/infocaGeoreferenced:true/);
assert.match(health,/infocaDirectIntegration:true/);
assert.match(health,/bombersEndpoint:true/);
assert.match(health,/bombersOfficialSource:true/);
assert.match(health,/bombersGeoreferenced:true/);
assert.match(health,/bombersDirectIntegration:true/);
assert.match(health,/bombersForestOnlyInSituation:true/);
assert.match(health,/bombersUnpublishedPhaseIsNotActive:true/);
assert.match(health,/bombersRuntimeCache:true/);
assert.match(health,/bombersRuntimeCacheTtlHours:6/);
assert.match(health,/bombersFreshCacheSeconds:60/);
assert.match(health,/infoarEndpoint:true/);
assert.match(health,/infoarOfficialPdf:true/);
assert.match(health,/infoarDirectIntegration:true/);
assert.match(health,/infoarStatusOfficial:true/);
assert.match(health,/infoarLocationApproximate:true/);
...
assert.match(health,/galiciaAbsenceConfirmsSafety:false/);
assert.match(health,/galiciaReportingThresholdHectares:20/);
assert.match(health,/galiciaLocationApproximate:true/);
assert.match(health,/galiciaLocationSource:'IGN · CartoCiudad'/);
assert.match(health,/galiciaRuntimeCache:true/);
assert.match(health,/galiciaReportMaxAgeHours:36/);
assert.match(health,/asturiasOfficialBulletins:true/);
assert.match(health,/asturiasDirectIntegration:true/);
assert.match(health,/asturiasCoverageComplete:false/);
assert.match(health,/asturiasAbsenceConfirmsSafety:false/);
assert.match(health,/asturiasLocationApproximate:true/);
assert.match(health,/asturiasLocationSource:'IGN · CartoCiudad'/);
assert.match(health,/asturiasRuntimeCache:true/);
assert.match(health,/asturiasReportMaxAgeHours:36/);
assert.match(health,/infomurOfficialPosts:true/);
assert.match(health,/infomurDirectIntegration:true/);
assert.match(health,/infomurCoverageComplete:false/);
assert.match(health,/infomurAbsenceConfirmsSafety:false/);
assert.match(health,/infomurLocationApproximate:true/);
assert.match(health,/infomurLocationSource:'IGN · CartoCiudad'/);
assert.match(health,/infomurRuntimeCache:true/);
assert.match(health,/infomurRuntimeCacheTtlHours:24/);
assert.match(health,/infomurIncidentMaxAgeHours:36/);
assert.match(health,/infomurApi:'WordPress REST'/);
...
assert.match(airQuality,/version:'4\.17\.1'/);
...
assert.match(situation,/data\.version='4\.17\.1'/);
...
assert.match(situation,/fetchBombers/);
assert.match(situation,/fetchInfoar/);
assert.match(situation,/fetchGalicia/);
assert.match(situation,/fetchAsturias/);
assert.match(situation,/fetchMurcia/);
assert.match(situation,/item\.region==='Andalucía'/);
assert.match(situation,/item\.region==='Cataluña'/);
assert.match(situation,/item\.region==='Aragón'/);
assert.match(situation,/item\.region==='Galicia'/);
assert.match(situation,/item\.region==='Principado de Asturias'/);
assert.match(situation,/item\.region==='Región de Murcia'/);
assert.match(situation,/confidenceForAbsence/);
assert.match(situation,/20 hectáreas/);
...
assert.match(infoca,/version:'4\.17\.1'/);
...
assert.match(bombers,/version:'4\.17\.1'/);
...
assert.match(bombersSource,/MAX_UNPHASED_AGE_MS/);
assert.match(bombersSource,/FASE NO PUBLICADA/);
assert.match(bombersSource,/otherVegetation/);
assert.match(bombersSource,/sourceType:'direct'/);
assert.match(bombersSource,/getCache/);
...
assert.match(infoar,/version:'4\.17\.1'/);
assert.match(infoar,/official:true/);
assert.match(infoar,/runtime:'nodejs'/);
assert.match(infoar,/if\(!response\)return webResponse/);
assert.match(infoarSource,/napif-pdf\/download/);
assert.match(infoarSource,/PDFParse/);
assert.match(infoarSource,/@napi-rs\/canvas/);
assert.match(infoarSource,/globalThis\.DOMMatrix/);
assert.match(infoarSource,/import\('pdf-parse'\)/);
assert.match(infoarSource,/import\('pdf-parse\/worker'\)/);
assert.match(infoarSource,/PDFParse\.setWorker\(getData\(\)\)/);
assert.match(infoarSource,/Incendios acaecidos en las últimas horas/);
assert.match(infoarSource,/INFOAR_INCIDENT_SECTION/);
assert.doesNotMatch(infoarSource,/if\(!entries\.length\)throw/);
assert.match(infoarSource,/d_muni_ine/);
assert.match(infoarSource,/locationApproximate:true/);
assert.match(infoarSource,/centro del término municipal/);
assert.match(infoarSource,/MAX_REPORT_AGE_MS/);
...
assert.match(galiciaSource,/coverageComplete:false/);
assert.match(galiciaSource,/confidenceForAbsence:false/);
assert.match(galiciaSource,/locationApproximate:true/);
assert.match(galiciaSource,/no el origen, frente ni perímetro/);
assert.match(galiciaSource,/MAX_REPORT_AGE_MS/);
assert.match(galiciaSource,/getCache/);
assert.match(galiciaSource,/última copia válida/);
assert.match(galiciaSource,/GLOBALSIGN_RSA_OV_SSL_CA_2018/);
assert.match(galiciaSource,/rootCertificates/);
assert.match(galiciaSource,/UNABLE_TO_VERIFY_LEAF_SIGNATURE/);
assert.doesNotMatch(galiciaSource,/rejectUnauthorized\s*:\s*false/);
assert.doesNotMatch(galiciaSource,/NODE_TLS_REJECT_UNAUTHORIZED/);
assert.match(asturiasSource,/112asturias\.es\/datos-incendios-forestales-asturias/);
assert.match(asturiasSource,/cartociudad\.es\/geocoder\/api\/geocoder/);
assert.match(asturiasSource,/coverageComplete:false/);
assert.match(asturiasSource,/confidenceForAbsence:false/);
assert.match(asturiasSource,/locationApproximate:true/);
assert.match(asturiasSource,/no el origen, frente ni perímetro/);
assert.match(asturiasSource,/MAX_REPORT_AGE_MS/);
assert.match(asturiasSource,/getCache/);
assert.match(asturiasSource,/última copia válida/);
assert.match(asturiasSource,/cross-border/);
assert.doesNotMatch(asturiasSource,/rejectUnauthorized\s*:\s*false/);
assert.doesNotMatch(asturiasSource,/NODE_TLS_REJECT_UNAUTHORIZED/);
assert.match(murciaSource,/noticias\.112rmurcia\.es\/wp-json\/wp\/v2\/posts/);
assert.match(murciaSource,/cartociudad\.es\/geocoder\/api\/geocoder/);
assert.match(murciaSource,/coverageComplete:false/);
assert.match(murciaSource,/confidenceForAbsence:false/);
assert.match(murciaSource,/locationApproximate:true/);
assert.match(murciaSource,/no el origen, frente ni perímetro/);
assert.match(murciaSource,/CURRENT_MAX_AGE_MS/);
assert.match(murciaSource,/ARCHIVE_MAX_AGE_MS/);
assert.match(murciaSource,/getCache/);
assert.match(murciaSource,/última copia válida/);
assert.match(murciaSource,/non-forest/);
...
assert.match(weather,/version:'4\.17\.1'/);
assert.equal(JSON.parse(pkg).version,'4.17.1');
...
assert.equal(JSON.parse(lock).version,'4.17.1');
...
console.log('Source integrity checks passed for FuegoCerca 4.17.1.');
