...
assert.match(health,/version:'4\.9\.1'/);
assert.match(danger,/AEMET/);
assert.match(danger,/version:'4\.9\.1'/);
...
assert.match(situation,/data\.version='4\.9\.1'/);
assert.match(weather,/version:'4\.9\.1'/);
assert.equal(JSON.parse(pkg).version,'4.9.1');
assert.equal(JSON.parse(pkg).name,'fuegocerca');
assert.equal(JSON.parse(lock).version,'4.9.1');
...
console.log('Source integrity checks passed for FuegoCerca 4.9.1.');
