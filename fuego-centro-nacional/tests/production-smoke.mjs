...
assert.equal(asturias?.mode,'integrated','Asturias must integrate the direct SEPA bulletins');
assert.equal(typeof asturias?.ok,'boolean','Asturias coverage must expose source health');
...
assert.equal(health.infomurOfficialPosts, true);
assert.equal(health.infomurDirectIntegration, true);
assert.equal(health.infomurCoverageComplete, false);
assert.equal(health.infomurAbsenceConfirmsSafety, false);
assert.equal(health.infomurLocationApproximate, true);
assert.equal(health.infomurLocationSource, 'IGN · CartoCiudad');
assert.equal(health.infomurRuntimeCache, true);
assert.equal(health.infomurRuntimeCacheTtlHours, 24);
assert.equal(health.infomurIncidentMaxAgeHours, 36);
assert.equal(health.infomurApi, 'WordPress REST');
